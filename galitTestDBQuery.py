...
join `api_flightitinerary` fitin on scans. `itin_id` = fitin. `id`
where  fitin. `record_locator` != ' '
and fitin. `id` > 200
and scans.`created_at` >=  NOW() - INTERVAL 1 DAY
GROUP BY scans.`scan_state`;
""")
...
if percentage > 10:
    print  str(percentage) +' percent of the searches failed'+ ' https://app.logz.io/kibana/index.html#/discover/Staging-Failed-Searches' + ' set logz.io time to: Last 24 hours '
else:
    print 'Great!'
